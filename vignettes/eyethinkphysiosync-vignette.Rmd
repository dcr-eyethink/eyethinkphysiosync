...
You can save out this data into separate .csv files with one command. It will put them in a folder called 'processed' by default, or you can specify a different name

```{r}
saveout_datalist(data)
```

AVERAGE DATA OVER pids FOR PD in MERGE
CHANGE THE SAVEOUT and READIN to use FREAD/WRITE


If you are just interested in the behaviour and the physiological response of individuals, you can stop here. Or, in the next step, we use the ed data to calculate the physiological synchrony between these signals using cross recurrence analysis.
